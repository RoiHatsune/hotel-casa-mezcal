...
// ✅ Usa fecha local del navegador (evita problemas de zona horaria)
...
export default function Dashboard() {
  ...
  const todayStr = getTodayLocal() // ✅ fecha local YYYY-MM-DD

  // ── Métricas generales ────────────────────────────────────────────────────
  // Órdenes de hoy usando fecha local
  ...
  // ✅ Pasadías de hoy usando campo date (ya es YYYY-MM-DD local)
  const todayPassRev  = daypasses
    .filter(p => p.date === todayStr && p.status !== 'cancelled')
    .reduce((s, p) => s + (p.total || 0), 0)

  const checkedIn = reservations.filter(r => r.status === 'checked_in').length

  // ── GRÁFICO 1: Productos más vendidos (pie) ───────────────────────────────
  const categoryData = (() => {
    const map = {}
  ...
  // ── GRÁFICO 1b: Ventas por día últimos 7 días ─────────────────────────────
  const salesByDay = last7.map(date => {
    const dayOrders = orders.filter(o => {
      const d = new Date(o.created_at)
      return o.status === 'paid' && d.toLocaleDateString('en-CA') === date
    })
    return {
      dia:      formatDay(date),
  ...
  // ── GRÁFICO 2: Hospedaje — reservas + pasadías por día ───────────────────
  const hospedajeByDay = last7.map(date => {
    ...
    // ✅ p.date ya es YYYY-MM-DD — comparación directa sin conversión
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
          ...
          <div>
            ...
            {categoryData.length === 0 ? (
              <div style={{ height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#7ab8a8', fontSize: 13 }}>Sin datos aún</div>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
            ...
            <div>
              ...
              {roomStatusData.length === 0 ? (
                <div style={{ height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#7ab8a8', fontSize: 13 }}>Sin habitaciones</div>
              ) : (
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
